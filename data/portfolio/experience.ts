import { Experiences } from '../../src/models/interfaces/Portfolio/Experiences';

export const experience: Experiences = {
  experiences: [
    {
      id: 'cdc5ab34-ac55-5101-bb27-c2a18656dd13',
      role: 'Software Engineer',
      company: 'Datadog',
      date: 'Jul 2024 - Ongoing',
      description: [
        {
          id: '923701f4-f1c3-5ca9-b229-dbd8cc7849f1',
          bulletPoint:
            'Developed and maintained IAST and RASP instrumentation in the dd-trace-java repository to enhance application security',
        },
        {
          id: 'd9017875-27fe-58ae-9d19-d46b35f90609',
          bulletPoint:
            'Authored and contributed to RFCs to define and align on instrumentation standards and cross-language test strategies',
        },
        {
          id: 'f401cb2e-b719-5f92-bd53-c3ca408b6a6e',
          bulletPoint:
            'Designed and executed comprehensive test suites in Python in the system-tests repository, ensuring consistent functionality across multiple libraries and programming languages',
        },
        {
          id: '1a1e213e-359a-5915-9954-38934d35848a',
          bulletPoint:
            'Implemented Java-based instrumentation to provide seamless integration with modern applications and frameworks',
        },
        {
          id: 'd89e798b-2be0-5322-9b43-8b284645ade3',
          bulletPoint:
            'Collaborated with cross-functional teams to validate instrumentation performance and compatibility in distributed environments',
        },
      ],
      links: [
        {
          id: '54b77d4b-4cd8-54f6-a830-ced8fc33c431',
          name: 'dd-trace-java',
          link: 'https://github.com/DataDog/dd-trace-java',
        },
        {
          id: '1d1cd127-acf7-51b4-a599-a50499c90912',
          name: 'system-tests',
          link: 'https://github.com/DataDog/system-tests',
        },
      ],
      companyLink: 'https://www.datadoghq.com/',
      tags: [
        {
          id: '6e58af15-063e-552f-9d9e-2b6cf744efff',
          tag: 'Java',
        },
        {
          id: '075bba23-be6c-5002-a01c-6ec2cbf9c413',
          tag: 'Python',
        },
        {
          id: '16f71e99-debc-59bb-9fe2-827a639f40c1',
          tag: 'IAST',
        },
        {
          id: '6ce50d23-b813-5ea5-a051-2e9cfaa4fe36',
          tag: 'RASP',
        },
        {
          id: '4de516c7-8cd8-5f70-a362-ea6bf5162d49',
          tag: 'Security',
        },
      ],
    },
    {
      id: 'f70bc224-29ff-599d-b660-b4bcd25228a2',
      role: 'Software Engineer',
      company: 'eDreams ODIGEO',
      date: 'Oct 2023 - Jun 2024',
      description: [
        {
          id: '647ec83b-e77e-5605-913b-8991005aad9f',
          bulletPoint:
            'Developing and maintaining Java-based JAX-RS microservices for the backend of the application',
        },
        {
          id: '4f668731-e77b-5a57-8ab4-cb3a2565808a',
          bulletPoint:
            'Designing and maintaining tables in Oracle DB NoSQL for Big Data information',
        },
        {
          id: '61c10141-fc25-5b1b-9b7d-5f1649b4dbad',
          bulletPoint:
            'Using monitoring tools such as Datadog to ensure platform performance',
        },
        {
          id: 'b93c82e1-cae8-5865-96ce-162b5e64130d',
          bulletPoint:
            'Using messaging platforms like Apache Kafka to share information between different sources and users',
        },
        {
          id: '481d8e5e-9c3e-5ba3-b913-68d3902b22ce',
          bulletPoint:
            'Testing applications by creating Unit Tests with JUnit and Mockito, and conducting A/B Testing',
        },
        {
          id: 'aad4daea-5306-569f-bb87-fea78512dcbf',
          bulletPoint:
            'Being in charge of the full development cycle end-to-end with tools like Docker, GCP, Jenkins, Jira, and Git',
        },
      ],
      links: undefined,
      companyLink: 'https://www.edreamsodigeo.com',
      tags: [
        {
          id: 'b139251c-aede-5be2-af90-07f8deb69895',
          tag: 'Java',
        },
        {
          id: '2e728416-a0b5-574e-ae82-b13327d3d425',
          tag: 'GCP',
        },
        {
          id: '4360d82d-4954-5d74-8597-58c880d9fd90',
          tag: 'Datadog',
        },
        {
          id: '7f61ba25-527f-5472-827a-1878a5f05b35',
          tag: 'Docker',
        },
        {
          id: '56f5cbbe-c4a0-5720-ac18-3c837acea64b',
          tag: 'Apache Kafka',
        },
      ],
    },
    {
      id: '2c90eff1-044d-5766-83e1-607acce7a79f',
      role: 'Back-End Developer',
      company: 'Minsait',
      date: 'Jan 2023 - Oct 2023',
      description: [
        {
          id: 'cf162469-cc87-5855-a979-048bd52ef64d',
          bulletPoint: 'Developing microservices using Java Spring Boot',
        },
        {
          id: '1ed6967e-3e9b-5d07-8a81-53f53a1ae599',
          bulletPoint:
            'Developing and maintaining software using Java Spring Boot and PostgreSQL',
        },
        {
          id: 'ab0a7a9a-4e11-5872-bfab-a554c6f6a59e',
          bulletPoint:
            "Designing and coding database tables to store the application's data",
        },
        {
          id: '8f70d494-c49e-59a1-b6ab-c3eea0748e1c',
          bulletPoint:
            'Backend development, deployment, and scalability using Docker, K8s and AWS',
        },
        {
          id: '9e5fce06-8150-5f50-b605-33c567cb4617',
          bulletPoint:
            'Designing and developing integration tests using Java Spock Framework',
        },
        {
          id: 'ea83fc46-efcf-53fb-a52f-1595d1146559',
          bulletPoint: 'Tracking issues and version control using Git',
        },
      ],
      links: [
        {
          id: '20401b60-7b6c-5558-9d8d-911ed43ee83f',
          name: 'Enagás GTS',
          link: 'https://www.gdogas.es/en/public-portal/home',
        },
      ],
      companyLink: 'https://www.minsait.com',
      tags: [
        {
          id: '09e5372c-3ceb-5d9f-9f25-9667c0218073',
          tag: 'Java Spring Boot',
        },
        {
          id: '7c5e69e6-9bde-5d43-ad28-c83bcf070a65',
          tag: 'AWS',
        },
        {
          id: '75b54072-739c-57cc-bcb0-87e390f41e19',
          tag: 'CI/CD',
        },
        {
          id: '84fc0edc-04ee-5777-8da0-1dcf4c839c24',
          tag: 'Docker',
        },
        {
          id: '0ff217a8-3ffb-58bf-8b86-9f12568f9bf2',
          tag: 'Java',
        },
      ],
    },
    {
      id: 'ed9c3d5d-d189-5642-a6e6-6c737f6664f2',
      role: 'Consultant',
      company: 'KPMG',
      date: 'Mar 2022 - Dec 2022',
      description: [
        {
          id: '5f3e9b2a-4b95-5c73-9628-2ee4cd22d020',
          bulletPoint:
            'Perform low-code development with Microsoft’s Power Platform-tools (Power Apps, Power Automate, Power BI, and Power Virtual Agent)',
        },
        {
          id: '7a0e644a-d79c-52df-889c-ba6dc2991ced',
          bulletPoint:
            'Customization of applications through the use of scripts, Azure Functions and/or Plugins',
        },
        {
          id: '83991a72-b61f-56e5-b4d8-b206215f8265',
          bulletPoint: 'Microsoft PCF development with React',
        },
        {
          id: '47faa32c-8438-5516-b466-4ba5038f5182',
          bulletPoint:
            'Configure the different technological solutions of Microsoft Dynamics ERP and CRM',
        },
        {
          id: 'fa217f8a-f089-5a83-b586-3f6a6c63ab68',
          bulletPoint:
            'Develop functional documents of the proposed solutions and training documents for end-users',
        },
      ],
      links: undefined,
      companyLink: 'https://www.kpmg.com',
      tags: [
        {
          id: '7f634073-9ec3-55fb-b761-1b8b666fe1b0',
          tag: 'React',
        },
        {
          id: '825b17b8-d078-5b54-88e1-055786336a3e',
          tag: 'TypeScript',
        },
        {
          id: 'e0bbb86d-eb58-5186-a9af-c92ee5c4b26c',
          tag: 'Microsoft Azure',
        },
        {
          id: '85667ae9-b005-5e42-a024-d2f530e18591',
          tag: 'JavaScript',
        },
        {
          id: 'df1ffe0d-7024-5902-9695-ba692be71cc9',
          tag: 'C#',
        },
      ],
    },
    {
      id: '521acbce-97bb-54da-8e5a-3534162065ea',
      role: 'Software Engineer Intern',
      company: 'AINIA',
      date: 'Feb 2021 - Jun 2021',
      description: [
        {
          id: '0b25be87-494d-56ef-a0ab-04c2c40a8aa3',
          bulletPoint: 'Software development using Java Spring Boot',
        },
        {
          id: 'e0412e84-76dd-5a6f-88ac-1756f00137f5',
          bulletPoint: 'Preparation of technical and research documentation',
        },
        {
          id: 'ff5ba628-487b-5d0b-bac5-7106498ad20c',
          bulletPoint: 'Development and maintenance of web services',
        },
        {
          id: 'b0b2dd29-bac9-5245-a580-1781dfefaff8',
          bulletPoint: 'Development of R+D+I activities',
        },
      ],
      links: undefined,
      companyLink: 'https://www.ainia.es',
      tags: [
        {
          id: 'd9ff9fe4-629f-538b-911b-4dce66f39634',
          tag: 'Java Spring Boot',
        },
        {
          id: '284de0e0-0aee-58bf-ba0c-607755741ccd',
          tag: 'CI/CD',
        },
        {
          id: '9ea99ba2-d469-5c44-803c-356ac5b522a1',
          tag: 'Java',
        },
      ],
    },
  ],
};
